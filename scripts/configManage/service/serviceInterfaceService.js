...
angular.module("klwkOmsApp")
    .factory('serviceInterfaceService', ["ApiService","toolsService",function(ApiService,toolsService){
        //获取身份验证
        ...
        var query = function(scope) {
            var url = "/BasicInformation/ServiceInterface/Get";
            var param = $.extend({
                body: JSON.stringify([{
                    "OperateType": 8,
                    "LogicOperateType": 0,
        ...
        var enabledItem = function (scope,type) {
            var url = "/BasicInformation/ServiceInterface/Enable";
            var ids = [];
            if(type == 'single'){
        ...
        var disabledItem = function (scope,type) {
            var url = "/BasicInformation/ServiceInterface/Disable";
            var ids = [];
            if(type == 'single'){
                ids.push(scope.activeItem.id);
            }else if(type == 'batch'){
