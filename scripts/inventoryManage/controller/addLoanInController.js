...
angular.module("klwkOmsApp")
    .controller("addLoanInController", ["$scope","$rootScope" ,"addLoanInService","APP_MENU","APP_DATA","toolsService",function($scope,$rootScope,addLoanInService,APP_MENU,APP_DATA,toolsService) {
        ...
        function init(){
            ...
            addLoanInService.queryWarehouseAll($scope);
            ...
            $scope.paginationProductConf = {
                ...
                getPageIndex:function (currentPage,itemsPerPage) {
                    ...
                    if($scope.paginationProductConf.totalItems / itemsPerPage  === currentPage){
                        ...
                        addLoanInService.queryUsers($scope);
                    }else{
            ...
            $scope.paginationSkuConf = {
                ...
                onChange: function () {	//操作之后的回调
                    addLoanInService.queryGoods($scope);
                }
            };
        ...
        function eventBind() {
            ...
            $scope.showModal = function (name) {
                ...
                if (name == 'selectUserModal') {
                    if (!$scope.userList) {
                        addLoanInService.queryUsers($scope, function () {
                            $(" #" + name).modal('show');
                        });
                    } else {
                        $(" #" + name).modal('show');
                    ...
                    if($scope.productFormDate.loanuser){
                        if (!$scope.tableSkuList) {
                            addLoanInService.queryGoods($scope, function () {
                                $(" #" + name).modal('show');
                            });
                        } else {
                            $(" #" + name).modal('show');
                        ...
                        toolsService.alertMsg({content : "请选择借调人!",time : 1000});
                    };
                }
                ;
            };
            ...
            $scope.productSearchEnsure = function () {
                addLoanInService.queryGoods($scope, 1, 6);
            };
            ...
            $scope.selectProduct = function (obj, e, type) {  //type 0 商品   1 规格  2组合
                ...
                if (type == 1) {
                    addLoanInService.getProductInventory($scope, obj);
                };
            ...
            $scope.addBuySubmit = function () {
                addLoanInService.loanInSave($scope);
            };
