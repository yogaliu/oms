/**
 * Created by yoga on 2017/5/17.
 */

angular.module('klwkOmsApp')
    .service('fbpQuitGoodsNoticeBillService', ['ApiService', 'toolsService', 'APP_MENU',"fbpPublicService",
        function (ApiService, toolsService, APP_MENU,fbpPublicService) {
            //配置参数
            var configData = {
                columns: [
                    {name: "状态", tag: "statusname"},
                    {name: "退货单号", tag: "returnordercode"},
                    {name: "通知单号", tag: "code"},
            ...
            var listInterface = {
                ...
                getOrderList: function (scope) {
                    var url="/B2B/B2BReturnOrderNotice/Query"
                    InterFace.getDatas(url,scope,configData.tbDatas(scope),function(res){
                        scope.orderListTbody = res.data;
                        //转换状态值
                        $.each(scope.orderListTbody,function(i,v){
                            for(var k in APP_MENU.B2BreturnSalesMemo){
                                if(v.status == k){
                                    v.statusname = APP_MENU.B2BreturnSalesMemo[k];
                                }
                            }
                        } );
