...
angular.module("klwkOmsApp")
    .controller("fbpNoticeBillController", ["$scope", "$rootScope", "$state", "WAP_CONFIG","toolsService","fbpPublicService","fbpNoticeBillService","APP_MENU",
        function ($scope, $rootScope, $state, WAP_CONFIG,toolsService,fbpPublicService,fbpNoticeBillService,APP_MENU) {
            var indexId="#fbpNoticeBill";
            fbpNoticeBillService.DomOperate.dominit($scope);
            //加载下拉框
            $('#fbpNoticeBill').selectPlug();
            //初始化getpage函数
            ...
            $scope.domOperate = {
                ...
                overOrder:function(list,$event){
                    ...
                    fbpNoticeBillService.Interface.overOrder($scope,list)
                },
                //高级搜索显示隐藏
                ...
                advanceSearchConfirm: function (myEvent) {
                    ...
                    //重新请求数据
                    fbpNoticeBillService.Interface.getOrderList($scope);
                    //高级搜索隐藏
                ...
                singleSearch:function($event){
                    if($event.keyCode==13){
                        fbpNoticeBillService.Interface.getSearchList($scope)
                    }
                }
            };
...
//目前最后条数
            ...
            //单据状态
            $scope.B2BrequisitionStatusConfigData = {
                title: '退款状态',
                // 用来存放用户选中的状态
                chosed: $scope.formChoseData,
                // filed 是对应的后台接口的字段
                filed: 'Status',
                list: APP_MENU['B2BrequisitionStatus']
            };
