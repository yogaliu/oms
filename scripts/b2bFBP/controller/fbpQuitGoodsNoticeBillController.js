...
angular.module("klwkOmsApp")
    .controller("fbpQuitGoodsNoticeBillController", ["$scope", "$rootScope", "$state", "WAP_CONFIG", "toolsService", "fbpQuitGoodsNoticeBillService","fbpPublicService","APP_MENU",
        function ($scope, $rootScope, $state, WAP_CONFIG, toolsService, fbpQuitGoodsNoticeBillService,fbpPublicService,APP_MENU) {
            //初始化
            fbpQuitGoodsNoticeBillService.DomOperate.dominit($scope);

                //加载下拉框
                $('#fbpQuitGoodsNoticeBill').selectPlug();
            ...
            $scope.domOperate = {
                ...
                advanceSearchConfirm: function (myEvent) {
                    ...
                    fbpQuitGoodsNoticeBillService.Interface.getOrderList($scope);
                    //高级搜索隐藏
                ...
                singleSearch:function($event){
                    if($event.keyCode==13){
                        fbpQuitGoodsNoticeBillService.Interface.getSearchList($scope)
                    }
                }
            }
           ...
           //目前最后条数
            ...
            //单据状态
            $scope.B2BreturnSalesMemoConfigData = {
                title: '单据状态',
                // 用来存放用户选中的状态
                chosed: $scope.formChoseData,
                // filed 是对应的后台接口的字段
                filed: 'Status',
                list: APP_MENU['B2BreturnSalesMemo']
            };
